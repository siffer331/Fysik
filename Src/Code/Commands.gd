...
func dealloc(args: Array, code: Code, exporting := false) -> Array:
	if not args[0] in Global.variables:
		return ["error", "variable does not excist"]
	Global.variables.erase(args[0])
	Global.emit_signal("variable_changed")
...
func to(args: Array, code: Code, exporting := false) -> Array:
	...
	value.simplify()
	if exporting:
		Global.exported.append("$"+res[1][0].to_latex()+"="+value.to_latex()+"$")
...
func find(args: Array, code: Code, exporting := false) -> Array:
	...
	while changed:
		...
		for formula in Global.formulas:
			var count := 0
			for v in Global.formulas[formula].versions:
				if v in found or v in Global.formulas[formula].defaults:
					count += 1
			if count == len(Global.formulas[formula].versions)-1:
				changed = true
				for v in Global.formulas[formula].versions:
					if v in Global.formulas[formula].defaults:
						Global.defaults[v] = Global.formulas[formula].defaults[v]
					if not v in found:
						found[v] = Global.formulas[formula].versions[v]
						order.append(v)
		if target in found:
			break
	var res := ""
	if target in found:
		...
		used.invert()
		for v in used:
			Global.exported.append("$"+v+"="+Code.equation_to_latex(found[v])+"$")
			for u in used:
				if u == v:
					break
